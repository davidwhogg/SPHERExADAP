...
The observing strategy is a calibrator's dream, as it moves, over time, every source on the sky onto many different points in the detector space multiple times throughout a two-year mission, and (impressively) making the data public as it goes.

Here we propose to execute four work packages (WPs) with the \textsl{SPHEREx} data:
...
\paragraph{WP2: Develop and measure new cosmological observables} that correspond to marked cross-correlation functions between the intensity field and galaxies, in real space. These can be seen as very sensitive, differential ``stacking'' methods.
...
\paragraph{WP3, WP4: Interpret these observables} in terms of both traditional astrophysical signals (WP3) and new physics signals (WP4) from dark-matter--photon interactions.
...
We note two particular works that share considerable overlap with our proposed work.
